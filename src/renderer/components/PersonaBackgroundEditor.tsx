...
const PLACEHOLDER = 'Without a background, the persona only supplies a name.';

function latestOf(versions: PersonaBackgroundVersion[]): PersonaBackgroundVersion | null {
  return versions.length ? versions.reduce((a, b) => (b.versionNumber > a.versionNumber ? b : a)) : null;
}

/**
 * Version-history editor for a persona's background field, adapted from FieldEditor.tsx: same
 * load/select/autosave/duplicate/delete flow against personaFieldVersions:* instead of
 * fieldVersions:*, keyed by `personaId` directly instead of a character field id (a persona has
 * exactly one versionable field, not several field types to pick from).
 */
export default function PersonaBackgroundEditor({ personaId }: Props) {
  const [versions, setVersions] = useState<PersonaBackgroundVersion[]>([]);
  ...
  async function load(preferViewedId?: string) {
    const list = await window.electronAPI.personaFieldVersions.getByPersona(personaId);
    setVersions(list);
    const target = list.find((v) => v.id === preferViewedId) ?? latestOf(list);
    selectViewed(target ?? null);
  }

  function selectViewed(version: PersonaBackgroundVersion | null) {
    flushPendingSave();
  ...
  async function handleSaveAsNewVersion() {
    ...
    flushPendingSave();
    const created = await window.electronAPI.personaFieldVersions.duplicate(viewedVersionId);
    await load(created.id);
  }

  async function handleDeleteVersion(versionId: string) {
    if (!confirm('Delete this version? This cannot be undone.')) return;
    await window.electronAPI.personaFieldVersions.delete(versionId);
    await load();
  }
  ...
  async function flushPendingSave() {
    ...
    setSaveState('saving');
    await window.electronAPI.personaFieldVersions.updateContent(pending.id, pending.content);
  ...
  return (
    <div className="card">
      <div className="field-editor-header">
        <span className="field-editor-title">
