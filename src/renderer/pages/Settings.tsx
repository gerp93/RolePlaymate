...
export default function Settings() {
  ...
  async function handleChooseParentFolder() {
    const picked = await window.electronAPI.dbLocation.browseParentFolder();
    if (!picked) return;
    setDbBusy(true);
    await window.electronAPI.dbLocation.set(picked);
  }
  ...
  return (
    <div>
      ...
      <div className="card">
        ...
        <p className="text-muted" style={{ marginTop: -8, fontSize: 13 }}>
          RolePlaymate keeps its SQLite database, portraits and spoken audio together in a folder named
          RolePlaymate_Data. Choose a parent folder (a synced one like OneDrive or Dropbox keeps an off-device
          copy) and RolePlaymate_Data is created inside it, so it can share that folder with other apps without
          their files mixing. Moving copies the database and the portraits and audio it uses; the originals are
          left where they were. Switching restarts RolePlaymate.
        </p>
        {dbLocation && (
          <>
            <div className="field">
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              ...
              <button className="btn" disabled={dbBusy} onClick={handleChooseParentFolder}>
      ...
      <div className="card">
        ...
        <p className="text-muted" style={{ marginTop: -8, fontSize: 13 }}>
          ...
          running on a different port, or on another machine on your network. Start / Stop and
          model unload live in{' '}
          <Link to="/hardpoint">Hardpoint</Link>, not here.
        </p>
        {ollamaHost && (
          <>
            <div className="field">
      ...
      <div className="card">
        ...
        <p className="text-muted" style={{ marginTop: -8, fontSize: 13 }}>
          ...
          . Start / Stop live in <Link to="/hardpoint">Hardpoint</Link>.
        </p>
        {chatterboxHost && (
          <>
            <div className="field">
