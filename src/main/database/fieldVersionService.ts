...
export class FieldVersionService {
  ...
  getVersionsByField(fieldId: string): CharacterFieldVersion[] {
    const rows = this.db
      ...
      .all(fieldId);
    return this.ensureLatestIsActive(fieldId, rows.map(rowToFieldVersion));
  }
  ...
  getVersionById(id: string): CharacterFieldVersion | null {
    const row = this.db
      ...
      .get(id);
    return row ? rowToFieldVersion(row) : null;
  }
  ...
  createVersion(input: CreateFieldVersionInput): CharacterFieldVersion {
    ...
    return transaction(this.db, () => {
      ...
      this.db
        ...
        .run(id, input.fieldId, nextVersionNumber, input.content ?? '', isFirst ? 1 : 0, now, now);
  ...
  duplicateVersion(versionId: string): CharacterFieldVersion {
    ...
    return transaction(this.db, () => {
      ...
      this.db
        ...
        .run(id, source.fieldId, nextVersionNumber, source.content, now, now);
